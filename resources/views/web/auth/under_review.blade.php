<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>استاذ محمد النجار</title>
    <meta name="theme-color" content="#001a65" />
    <meta name="msapplication-navbutton-color" content="#001a65" />
    <meta name="apple-mobile-web-app-status-bar-style" content="#001a65" />
    <meta name="author" content="Ahmed Eltaroun ( Master code )" />
    <meta name="MobileOptimized" content="320" />
    <meta property="og:title" content="الرئيسية" />
    <meta property="og:type" content="website" />
    <meta property="og:url" content="https://drsherifwaly.com" />
    <meta property="og:image" content="" />
    <meta property="og:site_name" content="drsherifwaly" />
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@200..1000&display=swap" rel="stylesheet">
    <link rel="shortcut icon" type="image/png" href="{{asset('web_files')}}/images/compass.png" />
    <!-- css files -->
    <link rel="stylesheet" href="{{asset('web_files')}}/assets/bootstrab/bootstrap.min.css" />
    <link rel="stylesheet" href="{{asset('web_files')}}/assets/fontawesome/css/all.min.css" />
    <link rel="stylesheet" href="{{asset('web_files')}}/css/404.css" />
</head>
<body style="height: 97vh">
    <div class="container">
      <div class="main">
        <a class="navbar-brand" href="{{url('')}}"
          ><img src="{{asset('web_files')}}/images/logo.png" alt="" width="250"
        /></a>
        <p> تم انشاء الحساب بنجاح في انتظار التفعيل بعد دفع حساب الشهر عن طريق فودافون كاش علي رقم <b style="color: #ff8601">01028141159</b></p>
        <h3>يمكنك استخدام الكود التالي <b style="color: #ff8601">{{$code}}</b> لتسجيل الدخول مرة اخري</h3>
       <div class="button">
         <button > <a href="{{url('')}}">الرئيسية</a></button>
       </div>
      </div>
    </div>
</body>
</html>
